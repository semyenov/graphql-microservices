...
const logger = createLogger({ service: 'introspect-schema' });

async function introspectSchema() {
  const GATEWAY_URL = process.env.GATEWAY_URL || 'http://localhost:4000/graphql';

  logger.info(`Introspecting schema from ${GATEWAY_URL}...`);
  ...
  try {
    ...
    logger.info(`✅ Schema introspection saved to ${outputPath}`);
    ...
    if (sdlResponse.ok) {
      ...
      if (sdlResult.data?._service?.sdl) {
        ...
        logger.info(`✅ Schema SDL saved to ${sdlPath}`);
      }
    }
  } catch (error) {
    logger.error('❌ Failed to introspect schema', error as Error);
    process.exit(1);
  }
}
