...
// Resolvers
const queryResolvers: GraphQLQueryResolvers<Context> = {
  ...
  userByUsername: async (_, { username }, context) => {
    const cached = await context.cacheService.get<GraphQLUser>(cacheKeys.userByUsername(username));
    if (cached) return cached;
...
const userResolvers: GraphQLUserResolvers<Context> = {
